...
def launchTracer(exec_cmd, args, fuzz_int_event: t.Event, fuzzer_error_event: t.Event = None):
    ...
    def replace_input_path(args, input_file_path):
        ret = list(map(convert_to_bytes, args))
    ...
    while new_inputs_found or not fuzz_int_event.is_set():
        ...
        for el in new_inputs:
            ...
            if args.no_fuzzing:
                ...
                argv_fuzz_marker_path = os.path.join(fuzz_dir, "argv_fuzz_marker")
                if os.path.exists(argv_fuzz_marker_path):
                    # This is required if argv has been fuzzed, because input files generated by the fuzzer will contain a nul-separated list
                    # of arguments as a first thing (ended by a double nul-byte)
                ...
                if args.argv_rand:
                    ...
                    argv_fuzz_marker_path = os.path.join(fuzz_dir, "argv_fuzz_marker")
                    if not os.path.exists(argv_fuzz_marker_path):
                        with open(argv_fuzz_marker_path, "w"):
                            pass
                else:
                    argv = replace_input_path(exec_cmd[1:], input_cpy_path)
...
def main():
    ...
    def adjust_executable(executable: list, fuzz_in: str, fuzz_argv: bool):
        '''
            Store every index containing "@@" into a global variable. In the tracer thread, those index will contain the
            input file path.
            Also, if there are fixed input files, store their paths so that it is possible to create a private copy of them for
            each execution launched by the tracer thread.
            Finally, if argv is to be fuzzed, copy the original initial testcases (so that we can restore them in their original form when 
            everything's finished), prepend the nul-byte (\\x00) separated list of arguments to the testcases and setup the fuzzer to preload 
            the argv fuzzing library.
        '''
        args = executable[1:]
    ...
    executable = adjust_executable(executable, FUZZ_IN, args.argv_rand)
