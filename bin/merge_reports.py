...
def merge_reports(tracer_out_path: str, ignored_addresses: Dict[str, Set[int]] = dict(), apply_string_filter: bool = True, report_unique_access_sets = False, ignore_if_no_overlap = True):
                ...
                for _ in range(len(ma_set)):
                    ...
                    if ma.accessType == AccessType.WRITE:
                        ...
                        if ma.memType == MemType.STACK:
                            ...
                            heap_name = "Heap"
                            mem_base = findLibByName(heap_name, cur_load_bases)
                            if mem_base is not None:
                                mem_base = int(mem_base, 16)
                            else:
                                addr = ma.accessedAddress.address
                                heap_name, mem_base = findLib(addr, cur_load_bases)
                                mem_base = int(mem_base, 16)
                        newSet.setMemLocation(ma.accessedAddress, ma.memType, mem_base, heap_name)
    ...
    for ia, access_set in partial_overlaps.items():
        ...
        for maSet in access_set:
            ...
            memType = "stack" if maSet.memType == MemType.STACK else maSet.heap_name
